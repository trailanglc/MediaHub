...
func (p *ConvertProcessor) ProcessTask(ctx context.Context, t *asynq.Task) error {
	...
	if row.Media.StorageKey == nil || *row.Media.StorageKey == "" {
		return p.fail(ctx, videos, job, row.Asset.ID, videoPID, videoID, "missing storage key")
	}
	...
	if err != nil {
		return p.fail(ctx, videos, job, row.Asset.ID, videoPID, videoID, err.Error())
	}
...
func (p *ConvertProcessor) fail(ctx context.Context, videos *repository.VideoRepository, job *repository.ConvertJob, assetID int64, videoPID uuid.UUID, videoID, msg string) error {
	rediscache.InvalidateStreamVideo(ctx, p.deps.StreamCache, videoPID)
	p.cleanupHLSPrefix(ctx, videoPID)
